...
function waitForInput() {
  var spawn = require('child_process').spawn,
    py    = spawn('python', ['screen_capture.py']);
...
function buyBinance(info) {
  ...
  max_digits = getMaxDigitsForSell(info.binanceFormatInfo);
  ...
  api.binance.marketBuy(info.coin.toUpperCase()+"BTC", shares, function(response) {
    ...
    // });
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.2)).toFixed(max_digits));
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.3)).toFixed(max_digits));
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.35)).toFixed(max_digits));
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.4)).toFixed(max_digits));
  });
}
